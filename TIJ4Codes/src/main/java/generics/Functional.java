...
public class Functional {
  ...
  public static void main(String[] args) {
    ...
    Integer result = reduce(li, new IntegerAdder());//li 里面求和 //new出来的这个对象IntegerAdder，有做一些事情的函数，称为函数对象，用到策略来执行这些函数
    print(result);

    result = reduce(li, new IntegerSubtracter());//li 里面求差
    print(result);

    print(filter(li, new GreaterThan<Integer>(4)));//li 里面求大于4的数据集

    print(forEach(li,
      new MultiplyingIntegerCollector()).result());//li 里面求积 

    print(forEach(filter(li, new GreaterThan<Integer>(4)),
      new MultiplyingIntegerCollector()).result());//li 里面求大于4的数据集，并在该集中求积

    MathContext mc = new MathContext(7);//配置7位小数点
    List<BigDecimal> lbd = Arrays.asList(
      new BigDecimal(1.1, mc), new BigDecimal(2.2, mc),
      new BigDecimal(3.3, mc), new BigDecimal(4.4, mc));
    BigDecimal rbd = reduce(lbd, new BigDecimalAdder());//和121行的IntegerAdder相比，
    print(rbd);
